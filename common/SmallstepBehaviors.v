...
(** Whole-program behaviors of smallstep semantics *)
...
(** * Forward and backward behavior preservation. *)

Section BEHAVIOR_PRESERVATION.

Definition forward_preservation (L1 L2: Smallstep.semantics) :=
  forall beh1, program_behaves L1 beh1 ->
    exists beh2, program_behaves L2 beh2 /\ behavior_improves beh1 beh2.

Lemma forward_preservation_refl: forall L,
  forward_preservation L L.
Proof.
  intros L b H. exists b. split; auto. apply behavior_improves_refl.
Qed.

Lemma forward_preservation_trans: forall L1 L2 L3,
  forward_preservation L1 L2 -> forward_preservation L2 L3 -> forward_preservation L1 L3.
Proof.
  intros. unfold forward_preservation. intros. apply H in H1 as [b2 []]. apply H0 in H1 as [b1 []]. exists b1. split. auto. eapply behavior_improves_trans; eauto.
Qed.

Definition backward_preservation (L1 L2: Smallstep.semantics) :=
  forall beh2, program_behaves L2 beh2 ->
    exists beh1, program_behaves L1 beh1 /\ behavior_improves beh1 beh2.

Lemma backward_preservation_refl: forall L,
  backward_preservation L L.
Proof.
  intros L b H. exists b. split; auto. apply behavior_improves_refl.
Qed.

Lemma backward_preservation_trans: forall L1 L2 L3,
  backward_preservation L1 L2 -> backward_preservation L2 L3 -> backward_preservation L1 L3.
Proof.
...
Theorem forward_simulation_preservation: forall L1 L2,
  forward_simulation L1 L2 ->
  forward_preservation L1 L2.
Proof.
  unfold forward_preservation. intros L1 L2 FS. destruct FS as [init order match_states S]. intros. inv H.
...
Proof.
  intros. exploit forward_simulation_preservation; eauto.
...
Theorem backward_simulation_preservation: forall L1 L2,
  backward_simulation L1 L2 ->
  backward_preservation L1 L2.
Proof.
  unfold backward_preservation. intros L1 L2 S beh2 H. destruct S as [index order match_states S]. inv H.
...
Proof.
  intros. exploit backward_simulation_preservation; eauto.
  intros [beh' [A B]]. destruct B.
  congruence.
...
Proof.
  intros; split; intros.
- (* L -> atomic L *)
  exploit forward_simulation_preservation. eapply atomic_forward_simulation. eauto.
