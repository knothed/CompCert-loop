...
Require Import AST Linking Smallstep SmallstepBehaviors.
...
Parameter print_Cminor: Z -> Cminor.program -> unit.
...
(** We define two translation functions for whole programs: one
  starting with a C program and one with a Cminor program. *)

Definition transf_cminor_to_asm (p: Cminor.program) : res Asm.program :=
  OK p
   @@ print (print_Cminor 1)
...
Definition transf_c_to_cminor (p: Csyntax.program) : res Cminor.program :=
  ...
  @@@ time "Cminor generation" Cminorgen.transl_program
   @@ print (print_Cminor 0).

(*
Definition transf_c_to_asm (p: Csyntax.program) : res Asm.program :=
  OK p
  @@@ transf_c_to_cminor
...
Definition passes_cminor_to_asm :=
...
Definition match_prog_c_to_cminor: Csyntax.program -> Cminor.program -> Prop :=
  pass_match (compose_passes passes_c_to_cminor).

Definition match_prog_cminor_to_asm: Cminor.program -> Asm.program -> Prop :=
  pass_match (compose_passes passes_cminor_to_asm).

(** The [transf_c_to_asm] function, when successful, produces
  assembly code that is in the [match_prog] relation with the source C program. *)

Theorem transf_c_to_cminor_match:
  forall p tp,
  transf_c_to_cminor p = OK tp ->
  match_prog_c_to_cminor p tp.
Proof.
  intros p tp T.
  unfold transf_c_to_cminor, time in T. simpl in T. rewrite ! compose_print_identity in T.
  destruct (SimplExpr.transl_program p) as [p1|e] eqn:P1; simpl in T; try discriminate.
  destruct (SimplLocals.transf_program p1) as [p2|e] eqn:P2; simpl in T; try discriminate.
  destruct (Cshmgen.transl_program p2) as [p3|e] eqn:P3; simpl in T; try discriminate.
  destruct (Cminorgen.transl_program p3) as [p4|e] eqn:P4; simpl in T; try discriminate.
  unfold match_prog_c_to_cminor; simpl.
...
Theorem transf_cminor_to_asm_match:
  forall p tp,
  transf_cminor_to_asm p = OK tp ->
  match_prog_cminor_to_asm p tp.
Proof.
  intros p4 tp T.
  unfold transf_cminor_to_asm, time in T. rewrite ! compose_print_identity in T. simpl in T.
  ...
  unfold match_prog_cminor_to_asm; simpl.
...
Theorem cstrategy_to_cminor_semantic_preservation:
  forall p tp,
  match_prog_c_to_cminor p tp ->
...
Proof.
  intros p tp M. unfold match_prog_c_to_cminor, pass_match in M; simpl in M.
Ltac DestructM :=
  match goal with
    [ H: exists p, _ /\ _ |- _ ] =>
  ...
  assert (F: forward_simulation (Cstrategy.semantics p) (Cminor.semantics p4)).
  {
  ...
  apply factor_forward_simulation. auto. eapply sd_traces. eapply Cminor.semantics_determinate.
  apply atomic_receptive. apply Cstrategy.semantics_strongly_receptive.
  apply Cminor.semantics_determinate.
Qed.

Theorem c_to_cminor_semantic_preservation:
  forall p tp,
  match_prog_c_to_cminor p tp ->
...
Proof.
  ...
  eapply sd_traces; eapply Cminor.semantics_determinate.
  apply factor_backward_simulation.
  apply Cstrategy.strategy_simulation.
  apply Csem.semantics_single_events.
  eapply ssr_well_behaved; eapply Cstrategy.semantics_strongly_receptive.
  exact (proj2 (cstrategy_to_cminor_semantic_preservation _ _ H)).
Qed.

Theorem cminor_to_asm_semantic_preservation:
  forall p tp,
  match_prog_cminor_to_asm p tp ->
  forward_simulation (Cminor.semantics p) (Asm.semantics tp) /\
  backward_simulation (Cminor.semantics p) (Asm.semantics tp).
Proof.
  intros p tp M. unfold match_prog_cminor_to_asm, pass_match in M; simpl in M.
  repeat DestructM. subst tp.
  assert (F: forward_simulation (Cminor.semantics p) (Asm.semantics p17)).
  {
  eapply compose_forward_simulations.
  ...
  split. auto. apply forward_to_backward_simulation. auto. apply Cminor.semantics_receptive. apply Asm.semantics_determinate.
Qed.
...
Theorem c_to_cminor_program_simulation:
  forall p tp,
  transf_c_to_cminor p = OK tp ->
  backward_simulation (Csem.semantics p) (Cminor.semantics tp).
Proof.
  intros. now apply c_to_cminor_semantic_preservation, transf_c_to_cminor_match.
Qed.

Theorem cminor_to_asm_program_simulation:
  forall p tp,
  transf_cminor_to_asm p = OK tp ->
  forward_simulation (Cminor.semantics p) (Asm.semantics tp) /\
  backward_simulation (Cminor.semantics p) (Asm.semantics tp).
Proof.
  intros. now apply cminor_to_asm_semantic_preservation, transf_cminor_to_asm_match.
Qed.
...
Theorem separate_transf_c_to_cminor_simulation:
  forall c_units cminor_units c_program,
  nlist_forall2 (fun cu tcu => transf_c_to_cminor cu = OK tcu) c_units cminor_units ->
  link_list c_units = Some c_program ->
  exists cminor_program,
      link_list cminor_units = Some cminor_program
   /\ backward_simulation (Csem.semantics c_program) (Cminor.semantics cminor_program).
Proof.
  intros.
  assert (nlist_forall2 match_prog_c_to_cminor c_units cminor_units).
  { eapply nlist_forall2_imply. eauto. simpl; intros. apply transf_c_to_cminor_match; auto. }
  assert (exists cminor_program, link_list cminor_units = Some cminor_program /\ match_prog_c_to_cminor c_program cminor_program).
  { eapply link_list_compose_passes; eauto. }
  destruct H2 as (cminor_program & P & Q).
  exists cminor_program; split; auto. apply c_to_cminor_semantic_preservation; auto.
Qed.

Theorem separate_transf_cminor_to_asm_simulation:
  forall cminor_units asm_units cminor_program,
  nlist_forall2 (fun cu tcu => transf_cminor_to_asm cu = OK tcu) cminor_units asm_units ->
  link_list cminor_units = Some cminor_program ->
  exists asm_program,
      link_list asm_units = Some asm_program
   /\ backward_simulation (Cminor.semantics cminor_program) (Asm.semantics asm_program).
Proof.
  intros.
  assert (nlist_forall2 match_prog_cminor_to_asm cminor_units asm_units).
  { eapply nlist_forall2_imply. eauto. simpl; intros. apply transf_cminor_to_asm_match; auto. }
  assert (exists asm_program, link_list asm_units = Some asm_program /\ match_prog_cminor_to_asm cminor_program asm_program).
  { eapply link_list_compose_passes; eauto. }
  destruct H2 as (asm_program & P & Q).
  exists asm_program; split; auto. apply cminor_to_asm_semantic_preservation; auto.
Qed.
